...
export function NotificationBell() {
  ...
  return (
    <div className="notification-bell" ref={dropdownRef} style={{ position: "relative" }}>
      <button
        onClick={handleOpen}
        aria-label={t("bell.aria.base") + (unreadCount > 0 ? " " + t("bell.aria.unread", { count: unreadCount }) : "")}
...
function NotificationItem({ notification: n }: { notification: Notification }) {
  ...
  const actorName = (n as unknown as { actor?: { display_name: string } }).actor?.display_name ?? t("bell.item.actor.fallback");
  const topicTitle = (n as unknown as { topic?: { title: string } }).topic?.title ?? t("bell.item.topic.fallback");

  const text =
    n.type === "build_on"
      ? t("bell.item.build_on", { actor: actorName, topic: topicTitle })
      : n.type === "reply"
      ? t("bell.item.reply", { actor: actorName, topic: topicTitle })
      : n.type === "session_reminder"
      ? t("bell.item.session_reminder", { topic: topicTitle })
      : t("bell.item.moderation", { topic: topicTitle });
