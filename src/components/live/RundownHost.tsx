      ...
      {ended ? <div style={{ textAlign: "center", padding: "4rem" }}><h1>{t("live.rundown.host.ended")}</h1></div> : (
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 300px", gap: "1.25rem" }}>
          ...
          <aside style={{ border: "1px solid var(--border-light)", borderRadius: 14, padding: "1rem", alignSelf: "start" }}>
            ...
            {editing && <div role="list" className="live-rundown-list live-rundown-live-editor">
              {drafts.map((draft, index) => <section role="listitem" className="live-rundown-card" aria-grabbed={draggedDraft === index} key={draft.localId} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (draggedDraft != null && draggedDraft !== index) moveDraft(draggedDraft, index - draggedDraft); setDraggedDraft(null); }} style={{ opacity: draggedDraft === index ? 0.65 : 1 }}>
                <div className="live-rundown-card-header"><button type="button" draggable className="live-icon-btn" aria-label={t("live.rundown.action.drag")} onDragStart={() => setDraggedDraft(index)} onDragEnd={() => setDraggedDraft(null)}>⋮⋮</button><strong>{index + 1}. {kindLabel[draft.kind]}</strong><button className="live-icon-btn" aria-label={t("live.rundown.action.moveUp")} disabled={index === 0} onClick={() => moveDraft(index,-1)}>↑</button><button className="live-icon-btn" aria-label={t("live.rundown.action.moveDown")} disabled={index === drafts.length-1} onClick={() => moveDraft(index,1)}>↓</button>{(draft.kind === "choice" || draft.kind === "scale") && <button type="button" className="live-chip" onClick={() => setDrafts(duplicateFollowUpDrafts(drafts, index))}>{t("live.rundown.action.followUp")}</button>}<button className="live-icon-btn" aria-label={t("live.rundown.action.delete")} onClick={() => setDrafts(drafts.filter((item) => item.localId !== draft.localId))}>✕</button></div>
